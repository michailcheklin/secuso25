...
Die Buffer-Overflow-Schwachstelle ist in `if (fgets(*data->ptr, 48, stdin) == NULL)` (ar.c, Z. 51). Dort werden 48 Bytes in data.ptr über `stdin` gelesen. `data->ptr` ist ein doppelter Pointer, und somit ist `*data->ptr` wegen `*dp->ptr = dp->buf;` (ar.c, Z. 26) immer ein einfacher Pointer auf `data->buf`. Der Datenbereich, auf den `data->ptr`, wenn es vollständig dereferenziert wurde, im Struct zeigt, nur 28 Bytes lang, wodurch über die eigentlichen Grenzen `data->buf` um 20 Bytes hinausgeschrieben werden kann. `data->buf` liegt im Speicher immer 32 Bytes vor `data->ptr`. Der doppelte Pointer `data->ptr` kann somit überschrieben werden. Der Speicher sieht, relativ zu P = Position von `data->ptr` so aus:
...
### Heap leaken
Da im C-Code kein Speicher alloziiert wird (z. B. über `malloc` oder `calloc`), und das Ziel ist, den Inhalt einer Datei auf der Konsole auszugeben, ist es nicht notwendig, den Heap zu leaken. Da schon Binary, `libc`, `ld-linux` und der Stack bei jeder Ausführung des Programms geleakt sind, ist auch hier schon gezeigt, dass ASLR gegen Informations-Leaks wirkungslos ist. Dennoch wird zur Vollständigkeit der Demonstration, dass ASLR gegen Memory Disclosure wirkungslos ist, der Heap geleakt. Bei mehreren Ausführungen wurde der Inhalt der Binary, `libc` und `ld-linux` inspiziert und nach einem Pointer gesucht, denn das Programm wissen muss, wie es in jeder Ausführung den Heap immer adressieren kann. 0x21a3c0 Bytes nach Beginn von `libc` befindet sich ab dem 96. Byte des Symbols `mp_` (in gdb: `mp_+96`) ein Pointer, der immer zum Beginn des Heaps zeigt. So ist auch der Heap geleakt.
...
Als Gadget 0 wird ein Syscall-Gadget verwendet. Hierfür wird in ausführbaren Speicherbereichen nach der Bytefolge `0f 05` gesucht, welche der Opcode für SYSCALL ist. Da `libc`-Funktionen wie execve, system und exit auf den entsprechenden Syscalls basieren, ist in `libc` mindestens ein Syscall enthalten. 

Dann, um Syscalls auszuführen, werden als Gadgets 1 bis 4 die Gadgets gewählt, die die Register RAX, RDI, RSI und RDX befüllen, mit so wenigen Seiteneffekten wie möglich. Bei Gadget 4 muss man neben dem Wert für RDX einen weiteren, beliebigen Wert angeben, der an das nicht mehr benutzte Register R12 geschrieben wird. 

Um mögliche Ausgaben für Syscalls weiterverwenden zu können, wurde als Gadget 6 eines mit der Anweisung `MOV RDI, RAX` gewählt. Da dort mit `JAE 0x5a25c` ein bedingter Sprung zu einer auf jeden Fall nicht gemappten Adresse vorkommt, und RDX schon über Gadget 4 gesetzt werden kann, wird Gadget 5 genutzt, um RCX so zu setzen, dass das `JAE 0x5a25c` im Gadget 6 nicht ausgeführt wird. Damit der Sprung nicht ausgeführt wird, muss der Wert in RCX größer als RDX sein. RAX kann später mit Gadget 1 nach Gadget 6 neu beschrieben werden.
...
RDI wird gesetzt, indem der String `./secret.log\0` nach der ROP-Chain auf dem Stack abgelegt wird. Da RBP geleakt ist, kann dieser String immer erreicht werden. Als Ausgabe enthält RAX den File-Descriptor der geöffneten Datei.
...
Um den File-Descriptor aus RAX nach RDI zu übertragen, wird Gadget 6 verwendet. Vor der Nutzung von Gadget 6 wird mit Gadget 5 RCX auf einen sehr hohen Wert gesetzt und mit Gadget 4 RDX auf einen sehr niedrigen Wert gesetzt, damit in Gadget 6 die Instruktion `JAE 0x5a25c` nicht ausgeführt wird. Mit Gadget 1 wird RAX neu beschrieben. Als Speicherort für die gelesenen Daten wird ein großer globaler Buffer in einem beschreibbaren Datenbereich genutzt. Da der gesamte virtuelle Adressraum unabhängig von ASLR und PIE in jeder Ausführung geleakt werden konnte (s. Aufg. 2.1), stehen alle schreibbaren Speicherbereiche des Prozesses zur Verfügung. Hierzu eignet sich beispielsweise der Buffer `password`, da er durch den ROP-Angriff nicht mehr regulär verwendet wird. 
...
Da der Stack Canary immer mit einem 00-Byte endet (Stack Canary % 256 = 0), wurde zunächst exakt so viele Zeichen eingegeben, um im Speicher den Stack Canary zu erreichen. Der Zeilenumbruch überschrieb das 00-Byte und so konnten die ersten Bytes des Stack Canary ausgegeben werden. Wurden nicht alle Bytes des Stack Canary ausgegeben, weil es ein 00-Byte in der Mitte hatte (z. B. 0xABCD00EFABCDEF00), wird erneut von der Position des letzten geleakten Byte weiter geleakt, bis alle 8 Bytes des Stack Canary geleakt sind. Diese Vorgehensweise funktioniert mehrfach, da nach der Eingabe des Leak-Payloads, durch anschließende Eingabe von y das Symbol `__stack_chk_fail` nicht erreicht wird, sondern zurück gesprungen wird.

Sind alle 8 Bytes des Stack Canary geleakt, wird der Payload aufgebaut. Nach dem Befüllen des Buffers kommt der gerade geleakte Stack Canary dorthin, wo der ursprüngliche Stack Canary war und danach die gewünschte Return-Adresse. Die Manipulation des Stacks wird von `__stack_chk_fail` nicht bemerkt, da der Stack Canary gleich geblieben ist und somit spawnt eine Shell.

# 4. 

## 4.1 Schreiben Sie einen Exploit, der in den ausgegebenen Code-Bytes nach sinnvollen ROP-Gadgets sucht und eine Chain generiert, die die secret_function mit korrekten Argumenten aufruft.
Die gegebene Code-Region befindet sich immer an der virtuellen Adresse 0x550000000 bis 0x550000fff. Dieser Bereich ist les- und ausführbar. Ist der Hardmode nicht aktiviert, wird zusätzlich die Funktionsadresse von secret_function an die virtuelle Adresse 0x550001000 geschrieben. Der zweite Bereich ist les- und schreibbar. Die Bytes werden nicht vollständig gleichverteilt zufällig generiert. Es werden häufiger C3-Bytes generiert, die für RET-Instruktionen stehen und Bytes im Bereich \x50-\x57 für PUSH (RAX|RBX|RCX|RDX|RDI|RSI|RBP|RSP) und Bytes im Bereich \x58-\x57 für POP (RAX|RBX|RCX|RDX|RDI|RSI|RBP|RSP). Es wird nach Gadgets für POP RDI, POP RSI, POP RDX, POP RCX gesucht, um die Argumente von secret_function zu suchen. Um secret_function aufzurufen, wird der Stack Pointer RSP durch POP RSP auf die Adresse umgeleitet, die die Adresse der secret_function enthält. Leider geschieht, sobald das Programm nach der Umleitung des Stack Pointers in der secret_function ist, beim Aufruf von puts() ein Segfault. Da die Aufgabe 4 eine optionale Aufgabe ist, werde ich in der nächsten praktischen Übungssitzung mir die korrekte Lösung anhören. 